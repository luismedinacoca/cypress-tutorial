describe(`Lecture 082 - Alert with Input value in ${Cypress.env("demoQA")}/alerts`, () => {
  beforeEach(() => {
      cy.visit(`${Cypress.env("demoQA")}/alerts`);
  });
      
  it('Debería hacer clic en el botón "Click for JS Alert" y aceptar la alerta', () => {
    cy.contains('button', 'Click for JS Alert').click();  // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
    ...
    cy.get('#result').should('have.text', 'You successfully clicked an alert');  // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
    cy.get('#result').should('have.css', 'color', 'rgb(0, 128, 0)')
  });
    
  it('Debería hacer clic en el botón "Click for JS Confirm" y aceptar la confirmación', () => {
    cy.contains('button', 'Click for JS Confirm').click();  // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
    cy.on('window:confirm', (message) => {
      expect(message).to.be.equal('I am a JS Confirm');
      return true; // Indica que se acepta la confirmación
    });
    cy.contains('You clicked: Ok').should('be.visible');   // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
    cy.get('#result').should('have.text', 'You clicked: Ok');
    cy.get('#result').should('have.css', 'color', 'rgb(0, 128, 0)');
  });
    
  it('Debería hacer clic en el botón "Click for JS Confirm" y cancelar la confirmación', () => {
    cy.contains('button', 'Click for JS Confirm').click(); // Selecciona el segundo botón  // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
    cy.on('window:confirm', (str) => {
      expect(str).to.equal('I am a JS Confirm');
      return false; // Indica que se cancela la confirmación
    });
    cy.contains('You clicked: Cancel').should('be.visible');   // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
  ...
  it('Debería hacer clic en el botón "Click for JS Prompt" y aceptar el prompt ingresando texto', () => {
    ...
    cy.window().then((window) => {
      cy.stub(window, 'prompt').returns(textoIngresado);
      cy.contains('button', 'Click for JS Prompt').click(); // Selecciona el tercer botón
    });
  ...
  it('Debería hacer clic en el botón "Click for JS Prompt" y aceptar el prompt sin ingresar texto (dejando el valor por defecto)', () => {
    cy.window().then((win) => {
      ...
      cy.contains('button', 'Click for JS Prompt').click(); // Selecciona el tercer botón   // TODO ⬅️ ⬅️ ⬅️ ⬅️ ⬅️  
