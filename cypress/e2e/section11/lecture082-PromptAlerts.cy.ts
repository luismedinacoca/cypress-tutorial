describe(`Lecture 082 - Alert with Input value in ${Cypress.env("theInternet")}/javascript_alerts`, () => {
  beforeEach(() => {
  ...
  it.only('Debería hacer clic en el botón "Click for JS Prompt" y aceptar el prompt ingresando texto', () => {
  ...
  it.only('Debería hacer clic en el botón "Click for JS Prompt" y aceptar el prompt sin ingresar texto (dejando el valor por defecto)', () => {
    cy.window().then((win) => {
  ...
  it.only('Debería hacer clic en el botón "Click for JS Prompt" y cancelar el prompt', () => {
    cy.window().then((win) => {
